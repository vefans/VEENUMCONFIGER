...
typedef enum {
    VECustomSwitchThumbJustifyLeft,
    VECustomSwitchThumbJustifyRight
} VECustomSwitchThumbJustify;

@interface VECustomSwitchThumb : UIView
@property (nonatomic, assign) BOOL isTracking;
-(void) growThumbWithJustification:(VECustomSwitchThumbJustify) justification;
-(void) shrinkThumbWithJustification:(VECustomSwitchThumbJustify) justification;
@end

@interface VECustomSwitchTrack : UIView
...
@interface VECustomSwitch () <UIGestureRecognizerDelegate>{
}
@property (nonatomic, strong) VECustomSwitchTrack* track;
@property (nonatomic, strong) VECustomSwitchThumb* thumb;
...
-(void) configureSwitch {
    ...
    // Initialization code
    if (!_track) {
        _track = [[VECustomSwitchTrack alloc] initWithFrame: self.bounds
    ...
    if (!_thumb) {
        _thumb = [[VECustomSwitchThumb alloc] initWithFrame:CGRectMake(kThumbOffset, kThumbOffset, self.bounds.size.height - 2 * kThumbOffset, self.bounds.size.height - 2 * kThumbOffset)];
...
- (void)setOn:(BOOL)on needDidChangeHandler:(BOOL)need animated:(BOOL)animated{
    ...
    if ([_delegate respondsToSelector:@selector(changeHandler:isOn:)] && need) {
        [_delegate changeHandler:self isOn:_on];
    }
}
- (void)setOn:(BOOL)on animated:(BOOL)animated {
    //Cancel notification to parent if attempting to set to current state
    [self setOn:on needDidChangeHandler:YES animated:animated];
}
...
-(void) setThumbIsTracking:(BOOL)isTracking {
    if (isTracking) {
        //Grow
        [self.thumb growThumbWithJustification: self.isOn ? VECustomSwitchThumbJustifyRight : VECustomSwitchThumbJustifyLeft];
    }
    else {
        //Shrink
        [self.thumb shrinkThumbWithJustification: self.isOn ? VECustomSwitchThumbJustifyRight : VECustomSwitchThumbJustifyLeft];
    }
...
@implementation VECustomSwitchThumb
...
-(void) growThumbWithJustification:(VECustomSwitchThumbJustify) justification {
    ...
    if (justification == VECustomSwitchThumbJustifyRight) {
...
@interface VECustomSwitchTrack ()
@property (nonatomic, weak) UIView* contrastView;
@property (nonatomic, weak) UIView* onView;
@end

@implementation VECustomSwitchTrack
...
-(id) initWithFrame:(CGRect)frame
    ...
    if (self = [super initWithFrame: frame]) {
        ...
        UIView *contrastView = [[UIView alloc] initWithFrame:contrastRect];
        _contrastView = contrastView;
        ...
        UIView *onView = [[UIView alloc] initWithFrame:frame];
        _onView = onView;
