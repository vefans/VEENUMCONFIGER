...
//
// Copyright © 2009-2016 iOS VESDK Team
//
...
@class VEBackgroundView;
@protocol VEProgressHUDDelegate;

typedef NS_ENUM(NSInteger, VEProgressHUDMode) {
    /// UIActivityIndicatorView.
    VEProgressHUDModeIndeterminate,
    /// A round, pie-chart like, progress view.
    VEProgressHUDModeDeterminate,
    /// Horizontal progress bar.
    VEProgressHUDModeDeterminateHorizontalBar,
    /// Ring-shaped progress view.
    VEProgressHUDModeAnnularDeterminate,
    /// Shows a custom view.
    VEProgressHUDModeCustomView,
    /// Shows only labels.
    VEProgressHUDModeText
};

typedef NS_ENUM(NSInteger, VEProgressHUDAnimation) {
    /// Opacity animation
    VEProgressHUDAnimationFade,
    /// Opacity + scale animation (zoom in when appearing zoom out when disappearing)
    VEProgressHUDAnimationZoom,
    /// Opacity + scale animation (zoom out style)
    VEProgressHUDAnimationZoomOut,
    /// Opacity + scale animation (zoom in style)
    VEProgressHUDAnimationZoomIn
};

typedef NS_ENUM(NSInteger, VEProgressHUDBackgroundStyle) {
    /// Solid color background
    VEProgressHUDBackgroundStyleSolidColor,
    /// UIVisualEffectView or UIToolbar.layer background view
    VEProgressHUDBackgroundStyleBlur
};

typedef void (^VEProgressHUDCompletionBlock)(void);
 ...
 * The VEProgressHUD window spans over the entire space given to it by the initWithFrame: constructor and catches all
 * user input on this region, thereby preventing the user operations on components below the view.
 *
 * @note To still allow touches to pass through the HUD, you can set hud.userInteractionEnabled = NO.
 * @attention VEProgressHUD is a UI class and should therefore only be accessed on the main thread.
 */
@interface VEProgressHUD : UIView
 ...
 * @param animated If set to YES the HUD will di sappear using the current animationType. If set to NO the HUD will not use
 ...
 */
+ (nullable VEProgressHUD *)HUDForView:(UIView *)view;
 ...
 */
@property (weak, nonatomic) id<VEProgressHUDDelegate> delegate;

/**
 * Called after the HUD is hiden.
 */
@property (copy, nullable) VEProgressHUDCompletionBlock completionBlock;
...
/**
 * VEProgressHUD operation mode. The default is VEProgressHUDModeIndeterminate.
 */
@property (assign, nonatomic) VEProgressHUDMode mode;
 ...
 */
@property (assign, nonatomic) VEProgressHUDAnimation animationType UI_APPEARANCE_SELECTOR;
...
@property (assign) BOOL isShowCancelBtn;
 ...
 */
@property (strong, nonatomic, readonly) VEBackgroundView *bezelView;

/**
 * View covering the entire HUD area, placed behind bezelView.
 */
@property (strong, nonatomic, readonly) VEBackgroundView *backgroundView;

/**
 * The UIView (e.g., a UIImageView) to be shown when the HUD is in VEProgressHUDModeCustomView.
...
@protocol VEProgressHUDDelegate <NSObject>
 ...
 */
- (void)hudWasHidden:(VEProgressHUD *)hud;
 ...
 */
@interface VERoundProgressView : UIView
 ...
 */
@interface VEBarProgressView : UIView
...
@interface VEBackgroundView : UIView

/**
 * The background style.
 * Defaults to VEProgressHUDBackgroundStyleBlur on iOS 7 or later and VEProgressHUDBackgroundStyleSolidColor otherwise.
 * @note Due to iOS 7 not supporting UIVisualEffectView, the blur effect differs slightly between iOS 7 and later versions.
 */
@property (nonatomic) VEProgressHUDBackgroundStyle style;
...
@interface VEProgressHUD (Deprecated)
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block completionBlock:(nullable VEProgressHUDCompletionBlock)completion __attribute__((deprecated("Use GCD directly.")));
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue
     completionBlock:(nullable VEProgressHUDCompletionBlock)completion __attribute__((deprecated("Use GCD directly.")));
...
@property (strong, nonatomic) UIColor *activityIndicatorColor __attribute__((deprecated("Use UIAppearance to customize UIActivityIndicatorView. E.g.: [UIActivityIndicatorView appearanceWhenContainedIn:[VEProgressHUD class], nil].color = [UIColor redColor];")));
