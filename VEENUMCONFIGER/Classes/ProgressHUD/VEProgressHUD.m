//
// VEProgressHUD.m
// Created by iOS VESDK Team on 2.4.09.
//
...
#define MBMainThreadAssert() NSAssert([NSThread isMainThread], @"VEProgressHUD needs to be accessed on the main thread.");
...
@interface VEProgressHUDRoundedButton : UIButton
@end


@implementation VEProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    VEProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    VEProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (VEProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (VEProgressHUD *)subview;
        }
    }
    return nil;
}
...
- (void)commonInit {
    ...
    _animationType = VEProgressHUDAnimationFade;
    _mode = VEProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(VEProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == VEProgressHUDAnimationZoom) {
        type = animatingIn ? VEProgressHUDAnimationZoomIn : VEProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == VEProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == VEProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == VEProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == VEProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    VEProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<VEProgressHUDDelegate> delegate = _delegate;
    if(_delegate){
...
- (void)setupViews {
    ...
    VEBackgroundView *backgroundView = [[VEBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = VEProgressHUDBackgroundStyleSolidColor;
    ...
    VEBackgroundView *bezelView = [VEBackgroundView new];
    ...
    if (_isShowCancelBtn) {
        ...
        CGRect backRect = CGRectMake(roundf((allRect.size.width - self.width) / 2) + self.xOffset,
                                     roundf((allRect.size.height - self.height) / 2) + self.yOffset, self.width, self.height);
        if(!_cancelBtn){
            _cancelBtn = [[UIButton alloc] initWithFrame:CGRectMake(backRect.origin.x + backRect.size.width - 22, backRect.origin.y - 22, 44, 44)];
            _cancelBtn.backgroundColor = [UIColor clearColor];
            _cancelBtn.imageEdgeInsets = UIEdgeInsetsMake((44-17)/2.0, (44-17)/2.0, (44-17)/2.0, (44-17)/2.0);
            [_cancelBtn setImage:[VEHelp imageWithContentOfFile:@"next_jianji/ProgressImage/进度取消默认_"] forState:UIControlStateNormal];
            [_cancelBtn setImage:[VEHelp imageWithContentOfFile:@"next_jianji/ProgressImage/进度取消点击_"] forState:UIControlStateHighlighted];
            [_cancelBtn addTarget:self action:@selector(cancelAction) forControlEvents:UIControlEventTouchUpInside];
            
        }else{
            _cancelBtn.frame = CGRectMake(backRect.origin.x + backRect.size.width - 22, backRect.origin.y - 22, 44, 44);
        }
...
#endif
    UIButton *button = [VEProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[VERoundProgressView class]];
    
    VEProgressHUDMode mode = self.mode;
    if (mode == VEProgressHUDModeIndeterminate) {
    ...
    else if (mode == VEProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[VEBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == VEProgressHUDModeDeterminate || mode == VEProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[VERoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == VEProgressHUDModeAnnularDeterminate) {
            [(VERoundProgressView *)indicator setAnnular:YES];
        }
    }
    else if (mode == VEProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == VEProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[VEProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[VEProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[VERoundProgressView class]]) {
        VERoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [VERoundProgressView appearanceWhenContainedIn:[VEProgressHUD class], nil];
#else
        appearance = [VERoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[VEProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((VERoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((VERoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[VEBarProgressView class]]) {
        VEBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [VEBarProgressView appearanceWhenContainedIn:[VEProgressHUD class], nil];
#else
        appearance = [VEBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[VEProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((VEBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((VEBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    VEBackgroundView *bezelView = self.bezelView;
...
- (void)setMode:(VEProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == VEProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
    ...
    if(_delegate != nil) {
        if(_delegate){
            if ([_delegate respondsToSelector:@selector(cancelDownLoad)]) {
                [_delegate performSelector:@selector(cancelDownLoad) withObject:self];
            }
        }
    }
}
...
@implementation VERoundProgressView
...
@implementation VEBarProgressView
...
@implementation VEBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = VEProgressHUDBackgroundStyleBlur;
            ...
            _style = VEProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(VEProgressHUDBackgroundStyle)style {
    if (style == VEProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = VEProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    VEProgressHUDBackgroundStyle style = self.style;
    if (style == VEProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == VEProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation VEProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [VEProgressHUD allHUDsForView:view];
    for (VEProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable VEProgressHUDCompletionBlock)completion {
...
- (BOOL)dimBackground {
    VEBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == VEProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    MBMainThreadAssert();
    self.backgroundView.style = VEProgressHUDBackgroundStyleSolidColor;
...
@implementation VEProgressHUDRoundedButton
