...
@interface VEMaskPasterView (Pentacle)<UIGestureRecognizerDelegate>

@end

@implementation VEMaskPasterView (Pentacle)
...
-(void)initPentacle:(float) CenterHeight atWidth:(float) CenterWidth
{
    ...
    UIPanGestureRecognizer* centreImageViewMoveGesture1 = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(centreImageViewMoveGesture_Pentacle:)];
    centreImageViewMoveGesture1.delegate = self;
    [self addGestureRecognizer:centreImageViewMoveGesture1];
    
    {
        UIView * view = [[VEOvalView alloc] initWithFrame:CGRectMake( (self.curretnView.frame.size.width - (RoundnessHeight + 60))/2.0, (self.curretnView.frame.size.height - (RoundnessHeight + 60))/2.0, RoundnessHeight + 60, RoundnessHeight + 60) atType:VEMaskType_PENTACLE atColor:self.mainColor];
        view.backgroundColor = [UIColor colorWithWhite:0.0 alpha:0.0];
//        view.layer.borderColor =  self.mainColor.CGColor;
//        view.layer.borderWidth = 1.5;
        self.pentacleCenterView = view;
    ...
    }
    {
        ...
        UIPanGestureRecognizer* rotateGesture = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(rotateGesture_Pentacle:)];
    ...
    UIPinchGestureRecognizer *GestureRecognizer = [[UIPinchGestureRecognizer alloc] initWithTarget:self action:@selector(pinchGestureRecognizer_Pentacle:)];
    [self addGestureRecognizer:GestureRecognizer];
    GestureRecognizer.delegate = self;

    UIRotationGestureRecognizer *rotation = [[UIRotationGestureRecognizer alloc] initWithTarget:self action:@selector(Rotation_GestureRecognizer_Pentacle:)];
...
//MARK: 平移
- (void) centreImageViewMoveGesture_Pentacle:(UIGestureRecognizer *) recognizer{
...
//MARK: 拖拽旋转
- (void) rotateGesture_Pentacle:(UIPanGestureRecognizer *) recognizer{

    touchLocation = [recognizer locationInView:self];
    
    if (recognizer.state == UIGestureRecognizerStateBegan) {
        currentCenter = self.pentacleCenterView.center;
        ...
        beginningPinch_x = self.pentacleCenterView.frame.size.width/2.0;
        beginningPinch_y = self.pentacleCenterView.frame.size.height/2.0;
        ...
        self.pentacleCenterView .frame = CGRectMake( (self.curretnView.frame.size.width - width)/2.0, (self.curretnView.frame.size.height - height)/2.0, width, height);
        [self setCenterView_Pentacle];
...
//MARK: 两指旋转
-(void)Rotation_GestureRecognizer_Pentacle:(UIRotationGestureRecognizer *)rotation
{
...
//MARK: 两指缩放
- (void)pinchGestureRecognizer_Pentacle:(UIPinchGestureRecognizer *)recognizer {
