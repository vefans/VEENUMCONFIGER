...
@interface VEMaskPasterView (Roundness)<UIGestureRecognizerDelegate>

@end

@implementation VEMaskPasterView (Roundness)
...
-(void)initRoundness:(float) CenterHeight atWidth:(float) CenterWidth
{
    ...
    self.roundnessView = view;
    self.curretnView = view;
    
    UIPanGestureRecognizer* centreImageViewMoveGesture1 = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(centreImageViewMoveGesture_Roundness:)];
...
//        view.layer.borderWidth = 1.5;
    ...
    }
    {
        ...
        UIPanGestureRecognizer* rotateGesture = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(rotateGesture_Roundness:)];
    ...
    }
    {
        ...
        UIPanGestureRecognizer* levelGesture = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(levelGestureGesture_Roundness:)];
    ...
    }
    {
        ...
        UIPanGestureRecognizer* verticalGesture = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(verticalGesture_Roundness:)];
    ...
    UIPinchGestureRecognizer *GestureRecognizer = [[UIPinchGestureRecognizer alloc] initWithTarget:self action:@selector(pinchGestureRecognizer_Roundness:)];
    [self addGestureRecognizer:GestureRecognizer];
    GestureRecognizer.delegate = self;

    UIRotationGestureRecognizer *rotation = [[UIRotationGestureRecognizer alloc] initWithTarget:self action:@selector(Rotation_GestureRecognizer_Roundness:)];
...
//MARK: 平移
- (void) centreImageViewMoveGesture_Roundness:(UIGestureRecognizer *) recognizer{
...
//MARK: 拖拽旋转
- (void) rotateGesture_Roundness:(UIPanGestureRecognizer *) recognizer{

    touchLocation = [recognizer locationInView:self];
    
    if (recognizer.state == UIGestureRecognizerStateBegan) {
        currentCenter = self.roundnessCenterView.center;
        ...
        beginningPinch_x = self.roundnessCenterView.frame.size.width/2.0;
        beginningPinch_y = self.roundnessCenterView.frame.size.height/2.0;
        ...
        self.roundnessCenterView .frame = CGRectMake( (self.curretnView.frame.size.width - width)/2.0, (self.curretnView.frame.size.height - height)/2.0, width, height);
        [self setCenterView_Roundness];
...
//MARK: 两指旋转
-(void)Rotation_GestureRecognizer_Roundness:(UIRotationGestureRecognizer *)rotation
{
...
//MARK: 两指缩放
- (void)pinchGestureRecognizer_Roundness:(UIPinchGestureRecognizer *)recognizer {
...
//MARK: 左右拉伸
- (void) levelGestureGesture_Roundness:(UIPanGestureRecognizer *) recognizer{
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan) {
        ...
        self.roundnessCenterView .frame = CGRectMake( (self.curretnView.frame.size.width - width)/2.0, (self.curretnView.frame.size.height - self.roundnessCenterView .frame.size.height)/2.0, width, self.roundnessCenterView .frame.size.height);
        [self setCenterView_Roundness];
        self.curretnView.transform = CGAffineTransformMakeRotation(-deltaAngle);
    }
...
//MARK: 上下拉伸
- (void) verticalGesture_Roundness:(UIGestureRecognizer *) recognizer{
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan) {
        ...
        self.roundnessCenterView.frame = CGRectMake( (self.curretnView.frame.size.width - self.roundnessCenterView.frame.size.width)/2.0, (self.curretnView.frame.size.height - height)/2.0, self.roundnessCenterView.frame.size.width, height);
        [self setCenterView_Roundness];
...
-(void)setCenterView_Roundness
{
    ...
    [((VEOvalView*)self.roundnessCenterView) setNeedsDisplay];
}
