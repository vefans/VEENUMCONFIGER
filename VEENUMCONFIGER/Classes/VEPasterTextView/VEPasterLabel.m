...
@interface VEPasterLabel(){
    float labelWidth;
    VETextLayer * textLayer;
}
@end

@implementation VEPasterLabel
...
//    [VEHelp solveUIWidgetFuzzyRect:viewFrame];
...
- (void)drawTextInRect:(CGRect)rect {
    if (_isUseAttributedText) {
        ...
        if(![_strokeColor isEqual:[UIColor clearColor]] && _strokeWidth > 0){
        ...
        if (_innerStrokeWidth > 0) {
            CGContextSetLineWidth(c, _innerStrokeWidth);
            CGContextSetLineJoin(c, kCGLineJoinMiter);
            CGContextSetTextDrawingMode(c, kCGTextStroke);
            self.textColor = [_innerStrokeColor colorWithAlphaComponent:_innerStrokeAlpha];
            [super drawTextInRect:rect];
        }
        if(_fontColor){
...
- (void)adjustsFontSizeToFillItsContents
{
    ...
    for (int i = CATEGORY_DYNAMIC_FONT_SIZE_MAXIMUM_VALUE; i > CATEGORY_DYNAMIC_FONT_SIZE_MINIMUM_VALUE; i--) {
        ...
        if (CGRectGetHeight(rectSize) <= CGRectGetHeight(self.frame)) {
            //            ((VEPasterTextView *)self.superview).fontSize = (CGFloat)i-1;
            break;
        }
    }
}
