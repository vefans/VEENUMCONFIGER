...
@interface VEICGVideoTrimmerView() <UIScrollViewDelegate>{
...
@implementation VEICGVideoTrimmerView
...
- (void)resetSubviews
{
    ...
    if (self.showsRulerView) {
        ...
        VEICGRulerView *rulerView = [[VEICGRulerView alloc] initWithFrame:rulerFrame widthPerSecond:self.widthPerSecond themeColor:self.themeColor];
...
- (void)notifyDelegate
{
    ...
    if(array.count>1){
        for (int i=0;i<array.count; i++) {
            VEComminuteRangeView *rangeView = array[i];
            ...
            if(rangeView.frame.origin.x<=self.scrollView.contentOffset.x && rangeView.frame.origin.x + rangeView.frame.size.width >=self.scrollView.contentOffset.x){
                ...
                if(duration > _scrollcurrentTime){
                    ...
                    float totalWidth = self.videoRangeView.frame.size.width - (arr.count-1)*10;
                    
                    VEComminuteRangeView *rangeView = arr[self.currentIndex];
...
- (void)refreshTrimmerViewImage:(NSArray *)times{
    @autoreleasepool {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            if( weakSelf.contentFile.filtImagePatch )
            {
                [weakSelf thumbS_refreshThumbwithImageTimes:self->thumbTimes];
            }
            else{
                NSUInteger end = (self->thumbTimes.count > kRefreshThumbMaxCounts) ? kRefreshThumbMaxCounts : times.count;
                if(self->thumbTimes.count>0){
                    NSArray *imageTimes = [self->thumbTimes subarrayWithRange:NSMakeRange(0,end)];
                    dispatch_async(dispatch_get_main_queue(), ^{
                        
                        [weakSelf refreshThumbwithImageTimes:imageTimes nextRefreshIndex:1 isLastArray:(self->thumbTimes.count > kRefreshThumbMaxCounts ? NO : YES)];
...
//
            for (int i = 0; i<imageTimes.count; i++) {
                ...
                UIImage* videoScreen = [VEHelp getThumbImageWithUrl: [NSURL fileURLWithPath:strPatch]];
                videoScreen = [self ImageCrop: videoScreen];
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    UIImage *  videoScreen1 = [VEHelp scaleImage:videoScreen toScale:(imageView.frame.size.height/videoScreen.size.height)];
                    
                    
                    [imageView setImage:videoScreen1];
                    if(videoScreen1) [self.allImages addObject:videoScreen1];
                    
                    if( i >= (self->thumbTimes.count - 1) )
                    {
                        UIImage *resultImage = [VEHelp drawImages:self.allImages size:CGSizeZero animited:NO];
                        
                        self.videoRangeView.image = resultImage;
                        ...
                        self.videoRangeView.frame = rect;
                        self.videoRangeView.contentMode = UIViewContentModeScaleAspectFill;
                        self.frameView.frame = self.videoRangeView.frame;
                        CGRect contentViewRect = CGRectMake(0, self.contentView.frame.origin.y,self.videoRangeView.frame.size.width + self.scrollView.frame.size.width , self.contentView.frame.size.height);
                        self.contentView.frame = contentViewRect;
                        self.videoRangeView.contentMode = UIViewContentModeScaleAspectFill;
                        [self.scrollView setContentSize:self.contentView.frame.size];
                        
                        self.videoRangeView.hidden = NO;
                        self.videoRangeView.backgroundColor = [UIColor clearColor];
                        self.loadImageFinish = YES;
                        [self.imageGenerator cancelAllCGImageGeneration];
                        self.imageGenerator.videoComposition = nil;
...
-(UIImage *)ImageCrop:(UIImage * ) image
{
    ...
    return [VEHelp image:image rotation:0 cropRect:rectCrop];
}
...
//                                                              }
                                                              ...
                                                              dispatch_async(dispatch_get_main_queue(), ^{
                                                                  ...
                                                                  if(videoScreen) [self.allImages addObject:videoScreen];
                                                          ...
                                                          if(isLastArray && index > self->thumbTimes.count - 1){
                                                              UIImage *resultImage = [VEHelp drawImages:self.allImages size:CGSizeZero animited:NO];
...
//                                                                  /([[UIScreen mainScreen] scale]>2 ? 2.0 :1.0)
                                                              ...
                                                              dispatch_async(dispatch_get_main_queue(), ^{
                                                                  ...
                                                                  if (self->thumbTimes.count > 0) {
                                                                      int counts = ceilf(self->thumbTimes.count/(float)kRefreshThumbMaxCounts);
                                                                      NSUInteger star = nextRefreshIndex*kRefreshThumbMaxCounts;
                                                                      NSUInteger end = (nextRefreshIndex == (counts - 1)) ? (self->thumbTimes.count-nextRefreshIndex*kRefreshThumbMaxCounts) : kRefreshThumbMaxCounts;
                                                                      NSRange range = NSMakeRange(star,end);
                                                                      NSArray *imageTimes = [self->thumbTimes subarrayWithRange:range];
                                                                      NSLog(@"initthumbImageGenerator1:%ld",(long)idx);
                                                                      self.imageGenerator = [weakSelf initthumbImageGenerator];
        ...
        for (int i=1; i<=[times count]; i++) {
            @autoreleasepool{
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    if(i == times.count){
                        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                            UIImage *resultImage = [VEHelp drawImages:_allImages size:CGSizeZero animited:NO];
                            dispatch_async(dispatch_get_main_queue(), ^{
...
- (NSMutableArray *)getTimesFor_videoRangeView{
    ...
    [arra sortUsingComparator:^NSComparisonResult(VEComminuteRangeView *obj1, VEComminuteRangeView *obj2) {
        /*
...
- (BOOL)cuttingVideo:(float)beforDuration{
    ...
    BOOL canTouch = NO;
    if(arr.count >=1){
        for (VEComminuteRangeView *rangeView in arr) {
    ...
    if( _comminuteRangeViewArray )
    {
        [_comminuteRangeViewArray removeAllObjects];
        _comminuteRangeViewArray = nil;
    }
    _comminuteRangeViewArray = [NSMutableArray new];
        ...
        if(animationed){
            for (VEComminuteRangeView *rangeView in arr) {
                ...
                if(rangeView.frame.origin.x<comminuteX && rangeView.frame.size.width+rangeView.frame.origin.x>comminuteX){
                    ...
                    //NSString *thumbPath = [cutThumbPath stringByAppendingPathComponent:[VEHelp createFilename]];
                    ...
                    VEComminuteRangeView *rangeVi = [[VEComminuteRangeView alloc] init];
                    ...
                    rangeVi.image = [VEHelp imageWithContentOfPath:thumbPath];
                    ...
                    VEComminuteRangeView *rangeVi2 = [[VEComminuteRangeView alloc] init];
...
//                    [_comminuteRangeViewArray removeObject:rangeView];
                    
                    [_comminuteRangeViewArray addObject:rangeVi];
                    [_comminuteRangeViewArray addObject:rangeVi2];
                    ...
                    if( _delegate && ( [_delegate respondsToSelector:@selector(trimageComminute:atFile:atRangeViewRect:)] ) )
                    {
                        [_delegate trimageComminute:rangeView.image atFile:rangeView.file atRangeViewRect:rangeView.frame];
                    }
        ...
        if( _delegate && ( [_delegate respondsToSelector:@selector(trimageComminute:atFile:atRangeViewRect:)] ) )
        {
            [_delegate trimageComminute:_videoRangeView.image atFile:nil atRangeViewRect:CGRectZero];
        }
